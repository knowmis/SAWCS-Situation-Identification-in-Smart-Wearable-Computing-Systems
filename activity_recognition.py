...
# The best parameters have been found with a GridSearchCV approach
...
# Define class names with updated names
class_names = sorted(y.unique())
class_names = [name.replace("LYING_DOWN", "lying_down").replace("SITTING", "sitting") for name in class_names]
...
# Load data
filename = 'Dataset_Selected_features_SA-WA_ActivityRecognition.csv'
...
# Perform 5-fold cross-validation and calculate training time
...
# Fit the classifier, predict, and calculate testing time
#start_time = time.time()
...
# Load data
df = pd.read_csv('First_Setof_Experiments/Context_SA+WA_Activity_13+13.csv')
X = df.iloc[:, :-1]
y = df.iloc[:, -1]
...
df.to_csv('First_Setof_Experiments/predicted_dataset_onelebeladded_giuseppe.csv', index=False)
# Print evaluation metrics
print(f'Accuracy: {mean_accuracy}')
print(f'Precision: {mean_precision}')
print(f'Recall: {mean_recall}')
...
#print(f'ROC AUC score: {mean_roc_auc}')
